...
export default function Sidebar({ isCreatorMode }: SidebarProps) {
  ...
  // Prüfen, ob der User die Rolle 'ADMIN' hat (case-insensitive)
  ...
  return (
    <aside className="hidden md:flex fixed left-0 top-16 bottom-0 w-64 flex-col border-r border-border bg-card">
      <nav className="flex-1 space-y-2 p-4 overflow-y-auto min-h-0">
        ...
        {isAdmin && (
          <>
            <div className="my-2 border-t border-border mx-4 opacity-50" />
            <button
              ...
              className={cn(
                ...
                location.pathname === '/admin'
                  ...
                  : 'text-foreground hover:bg-neutral hover:text-destructive'
              )}
            >
              <ShieldCheckIcon className="w-6 h-6 flex-shrink-0" strokeWidth={1.5} />
