                  ...
                  {clientSecret ? (
                      getStripe() ? (
                          ...
                          <div className="p-4 text-center border border-dashed border-border rounded-lg">
                            <p className="text-sm text-muted-foreground mb-2">Um Zahlungen zu tätigen, musst du zuerst die Cookies akzeptieren.</p>
                          </div>
                      )
                  ) : (
...
export default function PaymentModal(props: PaymentModalProps) {
  ...
  // Wenn kein Cookie-Consent da ist, zeigen wir eine Meldung statt des Bezahl-Modals
  if (!stripePromise) {
    return (
        <Dialog open={props.isOpen} onOpenChange={props.onClose}>
          <DialogContent className="bg-card text-card-foreground border-border max-w-md p-6 text-center">
            <p>Du musst den Cookies zustimmen, um Zahlungen durchführen zu können.</p>
          </DialogContent>
        </Dialog>
    );
  }
